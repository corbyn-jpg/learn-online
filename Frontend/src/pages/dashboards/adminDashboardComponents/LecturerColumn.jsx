            ...
            <div key={lecturer.id} className="flex items-stretch gap-0">
              ...
              <div className="flex flex-col items-center w-6 shrink-0">
                ...
                <div className={`w-2.5 h-2.5 rounded-full border-2 shrink-0 ${
                  lecturer.id === selectedLecturerId ? "border-[#3C0078] bg-[#3C0078]" : "border-gray-300 bg-white"
                }`} />
              ...
              >
                <div className={`w-9 h-9 rounded-full flex items-center justify-center shrink-0 ${
                  lecturer.id === selectedLecturerId ? "bg-white/20" : "bg-gray-100"
                }`}>
                  <User className={`w-4 h-4 ${lecturer.id === selectedLecturerId ? "text-white" : "text-gray-500"}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <p className={`text-[10px] uppercase tracking-wider font-semibold ${
                    lecturer.id === selectedLecturerId ? "text-white/70" : "text-gray-400"
                  }`}>
                    Lecturer
                  </p>
                  <p className={`text-sm font-bold truncate ${
                    lecturer.id === selectedLecturerId ? "text-white" : "text-gray-800"
                  }`}>
                    {lecturer.name}
                  </p>
                </div>
