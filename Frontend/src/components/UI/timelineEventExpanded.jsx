      ...
      <div className={`border rounded-2xl overflow-hidden transition-shadow duration-200 ${isPast ? "bg-gray-50 dark:bg-slate-800/50 border-gray-100 dark:border-slate-700" : "bg-white dark:bg-slate-800 border-gray-200 dark:border-slate-700 shadow-sm hover:shadow-md"}`}>
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-gray-100 dark:border-slate-700">
            ...
            <div className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full ${isPast ? "bg-gray-100 dark:bg-slate-700" : "bg-[#FF8731]/10"}`}>
              <MdAccessTime className={`w-4 h-4 ${isPast ? "text-gray-400 dark:text-slate-600" : "text-[#FF8731]"}`} />
              <span className={`text-sm font-semibold ${isPast ? "text-gray-400 dark:text-slate-600" : "text-[#FF8731]"}`}>{duration}</span>
            </div>
          </div>
        </div>
      </div>
