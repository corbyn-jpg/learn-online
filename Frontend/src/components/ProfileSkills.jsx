      ...
      {editMode ? (
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {skillGroups.map((group) => (
            <div
              key={group.key}
              className="rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-3 shadow-sm dark:from-slate-700 dark:to-slate-700"
            >
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {group.items.map((item, index) => (
                  <span
                    key={`${group.key}-${index}`}
                    className="rounded-full border border-slate-200 bg-white dark:bg-slate-600 px-2.5 py-1 text-xs font-medium text-slate-700 shadow-sm"
                  >
                    {item}
                  </span>
                ))}
