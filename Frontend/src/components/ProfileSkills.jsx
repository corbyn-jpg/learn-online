      ...
      {editMode ? (
        ...
        <div className="grid gap-3 md:grid-cols-3">
          {skillGroups.map((group) => (
            <div
              key={group.key}
              className="rounded-2xl border border-slate-200 bg-gradient-to-b from-slate-50 to-white p-3 shadow-sm dark:border-slate-700 dark:from-slate-800 dark:to-slate-900"
            >
              ...
              <div className="mt-2 flex flex-wrap gap-2">
                {group.items.map((item, index) => (
                  <span
                    key={`${group.key}-${index}`}
                    className="rounded-full border border-slate-200 bg-white px-2.5 py-1 text-xs font-medium text-slate-700 shadow-sm dark:border-slate-600 dark:bg-slate-800 dark:text-slate-100"
                  >
                    {item}
                  </span>
                ))}
